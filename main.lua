...
-- spritesheet png has 4 rows each of them have 6 images, so total of 24 images for animation
ExplosionSheetCols = 6
ExplosionSheetRows = 4
ExplosionScale = 2 -- spritesheet png instances are very small so they have to be scaled
ExplosionImageCount = ExplosionSheetCols * ExplosionSheetRows
...
function AnimatedExplosion:new(x, y, frames)
    ...
    Parameters:
        x - X coordinate
        y - Y coordinate
        frames - table containing all frames of explosion animation (now quads)
    Variables:
    ...
    self.frames = frames                -- expects a table of Quads
    self.x = x
    self.y = y
    self.frameIndex = 1
...
function AnimatedExplosion:draw()
    local idx = math.floor(self.frameIndex)
    local currentQuad = self.frames[idx]
    if currentQuad then
        local sheetW, sheetH = explosionSpritesheet:getDimensions()
        local fw = sheetW / ExplosionSheetCols
        local fh = sheetH / ExplosionSheetRows
...
function love.load()
    ...
    -- explosion_spritesheet.png is spritesheet which has multiple frames for explosion
    -- they are equally distanced from each other and are on multiple raws so I get height and width of png to
    explosionFrames = {}
    explosionSpritesheet = love.graphics.newImage("images/explosion_spritesheet.png")
    local sheetW, sheetH = explosionSpritesheet:getDimensions()
    local frameW = sheetW / ExplosionSheetCols 
    local frameH = sheetH / ExplosionSheetRows

    -- using nested for loop, one iterated over row second over column, each one gets frame for explosion animation
    for r = 0, ExplosionSheetRows - 1 do
        for c = 0, ExplosionSheetCols - 1 do
            local quad = love.graphics.newQuad(c * frameW, r * frameH, frameW, frameH, sheetW, sheetH)
